...
function gitCommit(message: string) {
    ...
    try {
        ...
        execFileSync('git', ['commit', '-m', message, '--allow-empty-message', '--no-verify'], {
            cwd: config.memoryDir,
            stdio: 'ignore',
            timeout: 5000,
        })
...
async function collectSessions(): Promise<{
    ...
    for (const child of children) {
        if (child.parentSession) {
            childCountMap.set(
                child.parentSession,
                (childCountMap.get(child.parentSession) || 0) + 1,
            )
        }
    }
            ...
            messages: [
                {
                    ...
                    content: [
                        {
                            type: 'text' as const,
                            text: `${sessions.length} sessions, $${totalCost.toFixed(2)} total cost:\n\n${listing}`,
                        },
                    ],
                    timestamp: Date.now(),
                },
...
async function getOrRebuildSummary(): Promise<string> {
    ...
    try {
        ...
        fs.writeFileSync(
            SUMMARY_CACHE,
            JSON.stringify({ summary: cachedSummary, timestamp: now }),
            'utf-8',
        )
    } catch {}
...
export default function (pi: ExtensionAPI) {
        ...
        ctx.ui.setWidget('memory-dashboard', (_tui: any, theme: any) => {
            ...
            container.render = (width: number) => {
                ...
                if (expanded) {
                    ...
                    if (openItems.length > 0) {
                        parts.push(
                            `${openItems.length} scratchpad item${openItems.length > 1 ? 's' : ''}`,
                        )
                    }
                    const hint = keyHint('app.tools.expand', 'to expand')
                    const line =
                        theme.fg('muted', parts.join(', ')) + ' ' + theme.fg('dim', `(${hint})`)
        ...
        if (!rebuildTimer) {
            rebuildTimer = setInterval(async () => {
                ...
                try {
                    ...
                    fs.writeFileSync(
                        SUMMARY_CACHE,
                        JSON.stringify({ summary: cachedSummary, timestamp: lastRebuildTime }),
                        'utf-8',
                    )
                } catch {}
            }, REBUILD_INTERVAL_MS)
    ...
    pi.on('session_before_compact', async (_event, ctx) => {
        ...
        if (hasMemory) {
            ctx.ui.notify(
                'Memory files available \u2014 consider persisting important context before compaction',
                'info',
            )
        }
    })
    ...
    pi.registerTool({
        ...
        parameters: Type.Object({
            target: StringEnum(['long_term', 'daily', 'note'] as const, {
                description:
                    "Where to write: 'long_term' for MEMORY.md, 'daily' for today's daily log, 'note' for notes/<filename>",
            }),
                ...
                if (!filename) {
                    return {
                        content: [
                            {
                                type: 'text',
                                text: "Error: 'filename' is required for target 'note'.",
                            },
                        ],
                        details: {},
                    }
        ...
        async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
            ...
            if (action === 'done' || action === 'undo') {
                if (!text) {
                    return {
                        content: [
                            { type: 'text', text: `Error: 'text' is required for ${action}.` },
                        ],
                        details: {},
                    }
                }
    ...
    pi.registerTool({
        ...
        parameters: Type.Object({
            target: StringEnum(
                ['long_term', 'scratchpad', 'daily', 'file', 'note', 'list'] as const,
                {
            ...
            filename: Type.Optional(
                Type.String({
                    description: "Filename for 'file' target (e.g. 'lessons.md', 'SOUL.md')",
                }),
            ),
        }),
        async execute(_toolCallId, params, _signal, _onUpdate, _ctx) {
            ...
            if (target === 'file') {
                if (!filename) {
                    return {
                        content: [
                            {
                                type: 'text',
                                text: "Error: 'filename' is required for target 'file'.",
                            },
                        ],
                        details: {},
                    }
                ...
                if (!content) {
                    return {
                        content: [{ type: 'text', text: `File not found: ${safe}` }],
                        details: {},
                    }
                }
                return {
                    content: [{ type: 'text', text: content }],
                    details: { path: filePath, filename: safe },
                }
            }
            ...
            if (target === 'note') {
                ...
                if (!content) {
                    return {
                        content: [{ type: 'text', text: `Note not found: notes/${safe}` }],
                        details: {},
                    }
                }
                return {
                    content: [{ type: 'text', text: content }],
                    details: { path: filePath, filename: `notes/${safe}` },
                }
            }
            ...
            if (target === 'scratchpad') {
                ...
                if (!content?.trim()) {
                    return {
                        content: [
                            { type: 'text', text: 'SCRATCHPAD.md is empty or does not exist.' },
                        ],
                        details: {},
                    }
                }
    ...
    pi.registerTool({
        ...
        async execute(_toolCallId, params) {
            ...
            return {
                ...
                details: {
                    query,
                    fileMatches: result.fileMatches.length,
                    lineMatches: result.lineResults.length,
                },
            }
        },
    })
